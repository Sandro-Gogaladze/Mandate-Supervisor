...
## Stage 10 — UI completion — **done, then revised on direction after live use**: the review page became the **case room** — one conversation that IS the case. The transcript renders the ledger (routing decisions, every briefing with its exact context expandable, findings, correlations, the report, signatures) interleaved with the officer's messages; a live block streams step/tool activity while a run is in flight; the gate renders inside the conversation. The orchestrator gained two caller-executed intents (run_triage, draft_report) so "run the review" / "draft the report" work as chat messages, and its reply is now a ledger event (orchestrator_replied). **Auto-triage on submission was removed** — a supervisor starts the first pass, from the room or by asking. Originally-scoped-but-not-built: Built: three agents wired, QuestionBox, CaseTimeline with verify-chain, PromptOverridesDialog, close-no-action, correlations and investigation answers in results, prioritised queue, report-on-demand, automatic rerun-to-directed-pass handoff. NOT built: the RunCompare view (GET /cases/{id}/runs/{a}/diff/{b} exists and is tested; UI pending).
