...
async def test_drafting_reuses_the_recorded_score_and_grounds_failures(store) -> None:
    fake = make_graph_fake()
    case_id = await _triaged(store, fake)
    _, _, state = await start_drafting(case_id, model=fake, store=store)
    ...
    assert {f.finding_id for f in record.findings if (f.severity_weight or 0) > 0} <= cited
    assert state["grounding_problems"] == []
    assert state["risk_score"] == record.risk_score
    assert sum(1 for e in store.events_for(case_id) if e.event_type == "score_computed") == 1
...
async def test_ungrounded_draft_still_reaches_the_reviewer(store) -> None:
    """Grounding is advisory, not a gate. It records what it found and the
    draft goes on to the human gate — an officer who asked for a report is
    never left with nothing to read, and nothing issues without their name."""
    ...
    _, _, state = await start_drafting(case_id, model=fake, store=store)
    # Still exactly one drafting call: the validator never regenerates.
    assert fake.call_log.count("draft_case_report") - drafts_before == 1
    assert state["report_blocked"] is False and state["grounding_problems"]
    (intr,) = state["__interrupt__"]
    assert intr.value["reason"] == "report_approval"
    record = project_case(store.events_for(case_id))
    assert record.report_blocked is False
    assert record.draft_report is not None and record.status == "pending_decision"
    grounding = [e for e in store.events_for(case_id) if e.event_type == "grounding_checked"]
    assert grounding and grounding[-1].payload["passed"] is False
    assert not [e for e in store.events_for(case_id) if e.event_type == "report_blocked"]
